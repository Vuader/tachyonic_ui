...
class Roles(object):
    ...
    def view(self, req, resp, role_id=None):
        if role_id is None:
            ...
            dt = datatable(req, 'roles', '/roles',
                           fields, view_button=True, service=False)
            ui.view(req, resp, content=dt, title='Roles')
        else:
            api = Client(req.context['restapi'])
            headers, response = api.execute(const.HTTP_GET, "/roles/%s" % (role_id,))
    ...
    def edit(self, req, resp, role_id=None):
        if req.method == const.HTTP_POST:
            ...
            api = Client(req.context['restapi'])
            headers, response = api.execute(const.HTTP_PUT, "/roles/%s" %
                                            (role_id,), form)
        else:
            api = Client(req.context['restapi'])
            headers, response = api.execute(const.HTTP_GET, "/roles/%s" % (role_id,))
    ...
    def create(self, req, resp):
        if req.method == const.HTTP_POST:
            try:
                ...
                api = Client(req.context['restapi'])
                headers, response = api.execute(const.HTTP_POST, "/roles", form)
                if 'id' in response:
                    id = response['id']
    ...
    def delete(self, req, resp, role_id=None):
        api = Client(req.context['restapi'])
        headers, response = api.execute(const.HTTP_DELETE, "/roles/%s" % (role_id,))
        self.view(req, resp)
